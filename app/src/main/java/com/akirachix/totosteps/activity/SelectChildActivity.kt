...
class SelectChildActivity : AppCompatActivity() {
    ...
    private fun checkChildAgeAndProceed(child: Child) {
        ...
        when {
            ageInMonths < 12 -> {
                showAgeRestrictedDialog("This autism assessment is only available for children who are one year old and above.")
            }
            ageInMonths > 36 -> {
                showAgeRestrictedDialog("This autism assessment is only available for children who are three years old and below.")
            }
            else -> {
    ...
    private fun calculateAgeInMonths(dateOfBirth: String): Int {
        try {
            ...
            Log.e("SelectChildActivity", "Error calculating age: ${e.message}")
            return 0
        }
    }

    private fun showAgeRestrictedDialog(message: String) {
